...
class PathInterpolator {
    ...
    reset(points_: ?Array<Point>, padding_: ?number) {
        ...
        const totalLength = this._distances[this._distances.length - 1];
        this.padding = Math.min(padding_ || 0, totalLength * 0.5);
        this.length = totalLength - this.padding * 2.0;
    }
    ...
    lerp(t: number): ?Point {
        ...
        const distToTarget = t * this.length + this.padding;
